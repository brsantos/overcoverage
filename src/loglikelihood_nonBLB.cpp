...
// Parallel worker class (fast, no Gamma/Omega allocations)
struct LogLikelihoodWorkerFast : public Worker {
  const arma::mat& y;
  const arma::mat& X;
  ...
  LogLikelihoodWorkerFast(const arma::mat& y,
                          const arma::mat& X,
                          const arma::vec& initial,
                          const arma::vec& first,
                          int L,
    ...
    : y(y), X(X), initial(initial), first(first), L(L), combins(combins), 
      ...
      probs1(probs1), probs2(probs2), logL(logL) {}
...
struct LogLikelihoodWorkerLegacy : public Worker {
  ...
  void operator()(std::size_t begin, std::size_t end){
    ...
    for (std::size_t j = begin; j < end; ++j) {
      ...
      arma::mat Omega1(8, obs_cols);
      arma::mat Omega2(8, obs_cols);
      ...
      for (int c = 0; c < obs_block; ++c) {
        Omega1(0, c) = probs1(c);
        Omega2(0, c) = probs2(c);
      }
      Omega1(1, obs_block) = 1;
      Omega1(2, obs_block + 1) = 1;
      Omega2(1, obs_block) = 1;
      Omega2(2, obs_block + 1) = 1;
      for (int c = 0; c < obs_block; ++c) {
        Omega1(6, obs_offset_6 + c) = probs1(c);
        Omega2(6, obs_offset_6 + c) = probs2(c);
      }
      ...
      for (int t = (first(j)-1); t < (T-1); ++t) {
        ...
        if (prev_k_index >= 0 && prev_k_index < obs_cols &&
            (prev_k_index != k_index || prev_k_false != k_false)) {
          ...
          if (prev_k_false >= 0 && prev_k_false < obs_cols && prev_k_false != prev_k_index) {
        ...
        int obs = static_cast<int>(y(j,t+1)) - 1;
        if (obs < 0 || obs >= obs_cols) {
          alpha1.zeros();
          alpha2.zeros();
          continue;
        }

        alpha1 %= Omega1.col(obs);
        alpha2 %= Omega2.col(obs);
      ...
      logL[j] = log(sum(individual_L));
    }
  }
};

// [[Rcpp::export]]
double loglikelihood_nonBLB_parallel(const arma::mat& y,
  ...
  // Create the worker
  LogLikelihoodWorkerFast worker(y, X, initial, first, L, combins, mixing_prop,
...
// [[Rcpp::export]]
double loglikelihood_nonBLB_parallel_legacy(const arma::mat& y,
                                     const arma::mat& covariates,
                                     const arma::mat& age2,
                                     const arma::mat& age3,
                                     const arma::mat& tin2,
  ...
  for (int j = 0; j < N; ++j) {
    ...
    int first_j = static_cast<int>(first(j));
    for (int t = first_j - 1; t < (T - 1); ++t) {
      int obs = static_cast<int>(y(j, t + 1)) - 1;
      int k = static_cast<int>(combins(j, t + 1));
      int k_index = k * reg_block - 1;
      int k_false = k * reg_block - half_block - 1;
    ...
    double L1 = sum(alpha1) * mixing_prop(0);
    double L2 = sum(alpha2) * mixing_prop(1);
    total_logL += log(L1 + L2);
  }
