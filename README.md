...
In order to learn about the number of *false positives*, i.e., the
number of individuals that are being considered over-covered, but
actually are still in the country, we need to save the indexes of these
values in another variable `cens_ind`, as being censored, so this is
